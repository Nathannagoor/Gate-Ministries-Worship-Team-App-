...
feedback/suggestions. Backed by Firebase (Firestore + Auth + Storage) so
everything syncs live between everyone who opens the site.
...
5. Go to **Project settings > General**, scroll to "Your apps", click the web icon (`</>`) to register a web app, and copy the config object it gives you.
...
commit it. Actual access control lives in `firestore.rules` and
`storage.rules`.
...
| `resources`     | any signed-in member (create), team leads (edit/delete) | files go to Firebase Storage under `resources/` |
...
- The design, layout, CSS and all UI logic are unchanged from the original
  ...
  same `db` / `userNs` / `assetsNs` interface, so the rest of the script
  didn't need to change.
- Chord sheet files are read as plain text in the browser and stored as
  text in Firestore (not uploaded to Storage) — same as the original.
  Only **Resources** uploads go to Firebase Storage.
